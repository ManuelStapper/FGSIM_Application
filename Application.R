...
paste0(apply(round(tempMat, digits = 1), 1, function(x){
  paste0(x, collapse = " & ")
}), collapse = "\\")

round(tempMat, digits = 2)
...
par(mfrow = c(1, 1))
hist(raster_df$value)

exp(c(-6, -4, -2, 0))
abline(v = log(c(0.001, 0.01, 0.1, 1)))



temp = risks
for(i in 1:length(current)) temp[, i] = temp[, i] * current[i]

# Without accounting for population size
values(riskRaster) = log(rowSums(temp) * (values(popS10) > 0))
plot(riskRaster)
# With accounting for population size
values(riskRaster) = log(rowSums(temp) * values(popS10))
plot(riskRaster)

temp = rowSums(temp)
temp = temp/max(temp)
values(riskRaster) = log(temp*values(popS10))
plot(riskRaster)



decay = matrix(nrow = 4, ncol = 6)
decay[1, ] = sapply(3:8, function(i) fits[[1]][[i]]$coefficients[startsWith(names(fits[[1]][[i]]$coefficients), "neweights.")][1])
decay[2, ] = sapply(3:8, function(i) fits[[2]][[i]]$coefficients[startsWith(names(fits[[2]][[i]]$coefficients), "neweights.")][1])
decay[3, ] = sapply(3:8, function(i) fits[[3]][[i]]$coefficients[startsWith(names(fits[[3]][[i]]$coefficients), "neweights.")][1])
decay[4, ] = sapply(3:8, function(i) fits[[4]][[i]]$coefficients[startsWith(names(fits[[4]][[i]]$coefficients), "neweights.")][1])

colnames(decay) = c("NGB", "B", "T", "G", "C", "R")
rownames(decay) = c("North", "East", "South", "West")
decay[, 6] = -decay[, 6]

round(decay, digits = 2)

decr_df = data.frame(region = character(24), model = character(24), decrease = numeric(24))
decr_df$model = rep(c("NGB", "Beeline", "Time", "Gravity", "Circle", "Radiation"), 4)
decr_df$region = rep(c("North", "East", "South", "West"), each = 6)
decr_df$decrease[decr_df$region == "North"] = 100 - tempMat[2, 2:7]/tempMat[2, 1]*100
decr_df$decrease[decr_df$region == "East"] = 100 - tempMat[6, 2:7]/tempMat[6, 1]*100
decr_df$decrease[decr_df$region == "South"] = 100 - tempMat[10, 2:7]/tempMat[10, 1]*100
decr_df$decrease[decr_df$region == "West"] = 100 - tempMat[14, 2:7]/tempMat[14, 1]*100
decr_df$model = factor(decr_df$model, levels = c("NGB", "Beeline", "Time", "Gravity", "Circle", "Radiation"))


ggplot(decr_df, aes(y = decrease, fill = model, x = region)) +
  geom_bar(stat = "identity", position = position_dodge(width = 0.9)) +  # Increase dodge width for more spacing
  scale_fill_manual(values = c(colsTemp, "red", "blue")) +                                 # Apply custom colors
  labs(x = "Distance Measure", y = "Decrease (in %)", fill = "Region") +
  theme_minimal() +
  theme(
...
# ggsave("Plots/Decrease.png", width = 8, height = 6)

# Different approach: For each district find the method that was best
...
models = c("NGB", "Beeline", "Time", "Gravity", "Circle", "Radiation")

temp_df = data.frame(model = models, decr = (1 - tempMat[2, 2:7] / tempMat[2, 1])*100)
o = rank(temp_df$decr)
temp_df$model = factor(models[o], models)
...
ggplot(temp_df,
  ...
  theme(
    ...
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    # axis.text.x = element_text(size = 20, color = cols[2]),
    # axis.title.x = element_text(size = 20, color = cols[2])
  ) + 
  annotate("text", x = 1, y = 1.2, label = models[o == 1], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 2, y = 1.2, label = models[o == 2], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 3, y = 1.2, label = models[o == 3], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 4, y = 1.2, label = models[o == 4], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 5, y = 1.2, label = models[o == 5], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 6, y = 1.2, label = models[o == 6], color = "white", size = 8, hjust = 0)
ggsave("Plots/PerformanceNorth.png", width = 6, height = 3)



temp_df = data.frame(model = models, decr = (1 - tempMat[6, 2:7] / tempMat[6, 1])*100)
o = rank(temp_df$decr)
temp_df$model = factor(models[o], models)
...
ggplot(temp_df,
  ...
  theme(
    ...
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    # axis.text.x = element_text(size = 20, color = cols[1]),
    # axis.title.x = element_text(size = 20, color = cols[1])
  ) + 
  annotate("text", x = 1, y = 1.2, label = models[o == 1], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 2, y = 1.2, label = models[o == 2], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 3, y = 1.2, label = models[o == 3], color = "white", size = 8, hjust = 0)+ 
  ...
  annotate("text", x = 5, y = 1.2, label = models[o == 5], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 6, y = 1.2, label = models[o == 6], color = "white", size = 8, hjust = 0)
ggsave("Plots/PerformanceEast.png", width = 6, height = 3)

temp_df = data.frame(model = models, decr = (1 - tempMat[10, 2:7] / tempMat[10, 1])*100)
o = rank(temp_df$decr)
temp_df$model = factor(models[o], models)
...
ggplot(temp_df,
  ...
  theme(
    ...
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    # axis.text.x = element_text(size = 20, color = cols[3]),
    # axis.title.x = element_text(size = 20, color = cols[3])
  ) + 
  annotate("text", x = 1, y = 1.2, label = models[o == 1], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 2, y = 1.2, label = models[o == 2], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 3, y = 1.2, label = models[o == 3], color = "white", size = 8, hjust = 0)+ 
  ...
  annotate("text", x = 5, y = 1.2, label = models[o == 5], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 6, y = 1.2, label = models[o == 6], color = "white", size = 8, hjust = 0)
ggsave("Plots/PerformanceSouth.png", width = 6, height = 3)


temp_df = data.frame(model = models, decr = (1 - tempMat[14, 2:7] / tempMat[14, 1])*100)
o = rank(temp_df$decr)
temp_df$model = factor(models[o], models)
...
ggplot(temp_df,
  ...
  theme(
    ...
    axis.title = element_blank(),
    axis.ticks = element_blank(),
    # axis.text.x = element_text(size = 20, color = cols[4]),
    # axis.title.x = element_text(size = 20, color = cols[4])
  ) + 
  annotate("text", x = 1, y = 1.2, label = models[o == 1], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 2, y = 1.2, label = models[o == 2], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 3, y = 1.2, label = models[o == 3], color = "white", size = 8, hjust = 0)+ 
  ...
  annotate("text", x = 5, y = 1.2, label = models[o == 5], color = "white", size = 8, hjust = 0)+ 
  annotate("text", x = 6, y = 1.2, label = models[o == 6], color = "white", size = 8, hjust = 0)
ggsave("Plots/PerformanceWest.png", width = 6, height = 3)



plot(Ger$geometry[ID2Geom$GeomID1],
     col = cols[bestModel])

temp = table(bestModel)
names(temp) = c("N", "NGB", "B", "T", "G", "C", "R")



W1 = getWeights(fits$S[[3]])
beta = fits$S[[3]]$coefficients
nefe1 = exp(beta[startsWith(names(beta), "ne.1")])

W2 = getWeights(fits$S[[7]])
beta = fits$S[[7]]$coefficients
nefe2 = exp(beta[startsWith(names(beta), "ne.1")])

which(startsWith(ID2Geom$Name[indS], "Donau"))
...
# Only looking at model 7 (circle)
...
wEpi = list()
for(i in 1:4){
  indCoef = which(startsWith(names(fits[[i]][[7]]$coefficients), "end.1"))
  endFE[[i]] = fits[[i]][[7]]$coefficients[indCoef]
  endFE[[i]][endFE[[i]] < -10] = -10
  wEnd[[i]] = (endFE[[i]]  - min(endFE[[i]] ))/(max(endFE[[i]] ) - min(endFE[[i]]))
  wEnd[[i]] = wEnd[[i]]*0.95 + 0.05
  indCoef = which(startsWith(names(fits[[i]][[7]]$coefficients), "ne.1"))
  epiFE[[i]] = fits[[i]][[7]]$coefficients[indCoef]
  wEpi[[i]] = (epiFE[[i]]  - min(epiFE[[i]] ))/(max(epiFE[[i]] ) - min(epiFE[[i]]))
  wEpi[[i]] = wEpi[[i]]*0.95 + 0.05
}

wgtsEnd = sapply(1:401, function(i){
  if(i %in% indN) return(wEnd[[1]][indN == i])
  if(i %in% indE) return(wEnd[[2]][indE == i])
  if(i %in% indS) return(wEnd[[3]][indS == i])
  if(i %in% indW) return(wEnd[[4]][indW == i])
})

wgtsEpi = sapply(1:401, function(i){
  if(i %in% indN) return(wEpi[[1]][indN == i])
  if(i %in% indE) return(wEpi[[2]][indE == i])
  if(i %in% indS) return(wEpi[[3]][indS == i])
  if(i %in% indW) return(wEpi[[4]][indW == i])
})

shp_sf$wgtsEnd = 1 - wgtsEnd
shp_sf$wgtsEpi = 1 - wgtsEpi


p1 = ggplot(data = shp_sf) +
  geom_sf(aes(fill = colsTemp, alpha = wgtsEnd), lwd = 0.2, show.legend = F) +
...
p2 = ggplot(data = shp_sf) +
  geom_sf(aes(fill = colsTemp, alpha = wgtsEpi), lwd = 0.2, show.legend = F) +
  scale_fill_manual(values = cols) +
  theme_minimal() +
  ggtitle("Epidemic") + 
  theme(
...
plot(p)
ggsave("Plots/FixedEffects.png", plot = p, width = 12, height = 8, dpi = 300)


# library("data.table")
forecast2 = as.data.table(forecast2)

lWIS7 = lapply(ind, function(x) rep(0, length(x)))
for(r in 1:4){
  for(d in 1:length(ind[[r]])){
    lWIS7[[r]][d] = sum(forecast2[model == 7 & h == 1 & type == 6 & region == r & district == d]$value)
  }
}

lWIS3 = lapply(ind, function(x) rep(0, length(x)))
for(r in 1:4){
  for(d in 1:length(ind[[r]])){
    lWIS3[[r]][d] = sum(forecast2[model == 3 & h == 1 & type == 6 & region == r & district == d]$value)
  }
}


mWIS = min(log(unlist(lWIS7)))
MWIS = max(log(unlist(lWIS7)))
lWISscl7 = lapply(lWIS7, function(x) (log(x) - mWIS)/(MWIS - mWIS))

mWIS = min(log(unlist(lWIS3)))
MWIS = max(log(unlist(lWIS3)))
lWISscl3 = lapply(lWIS3, function(x) (log(x) - mWIS)/(MWIS - mWIS))


wgtsWIS7 = sapply(1:401, function(i){
  if(i %in% indN) return(lWISscl7[[1]][indN == i])
  if(i %in% indE) return(lWISscl7[[2]][indE == i])
  if(i %in% indS) return(lWISscl7[[3]][indS == i])
  if(i %in% indW) return(lWISscl7[[4]][indW == i])
})

wgtsWIS3 = sapply(1:401, function(i){
  if(i %in% indN) return(lWISscl3[[1]][indN == i])
  if(i %in% indE) return(lWISscl3[[2]][indE == i])
  if(i %in% indS) return(lWISscl3[[3]][indS == i])
  if(i %in% indW) return(lWISscl3[[4]][indW == i])
})

shp_sf$wgtsWIS3 = wgtsWIS3
shp_sf$wgtsWIS7 = wgtsWIS7

p1 = ggplot(data = shp_sf) +
  geom_sf(aes(fill = colsTemp, alpha = wgtsWIS3), lwd = 0.2, show.legend = F) +
  scale_fill_manual(values = colsLight) +
  ggtitle("Power Law") +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
...
p2 = ggplot(data = shp_sf) +
  geom_sf(aes(fill = colsTemp, alpha = wgtsWIS7), lwd = 0.2, show.legend = F) +
  scale_fill_manual(values = colsLight) +
  ggtitle("Circle") +
  theme_minimal() +
  theme(
...
# Last observed cases in forecast windows: 928:1028

# Extract the mixture order
m = 1
r = 1
model = c("B", "T", "G", "C", "R")[m]
region = c("N", "E", "S", "W")[r]
temp = get(paste0("pars", m, "_", region))
temp2 = apply(expand.grid(1:length(ind[[r]]), 1:length(ind[[r]])), 1, function(x){
  sum(temp$w[x[1], x[2], ] > 0)
})
temp2 = temp2[temp2 > 0]
table(temp2)/length(temp2)


fits$N[[4]]$control$ne$weights$w(1.6, O[indN, indN])


# Ranked table for different forecast horizons
# Plot (?) for interval level
# Map for performance of best model compared to power law



# Types: 0 = Mean, (0, 1) = quantiles, 2 = WIS, 3 = WIS_D, 4 = WIS_U, 5 = WIS_O
#        6 = LIS, 7 = LIS_D, 8 = LIS_U, 9 = LIS_O, 10 = RPS

# Prepare results here

sapply(1:8, function(i){
  ind = (forecast2$h == 1) & (forecast2$model == i) & (forecast2$type == 6) & (forecast2$region == 1)
  mean(forecast2$value[ind])*100
})

# Create table from here

temp1 = forecast2$value[(forecast2$type == 7) & (forecast2$model == 2) & (forecast2$h == 1)]
temp2 = forecast2$value[(forecast2$type == 7) & (forecast2$model == 3) & (forecast2$h == 1)]
...
# Detailled look into coefficients

ind = startsWith(names(fits[[26]]$coefficients), "end.1.")
w = fits[[26]]$coefficients[ind]
w[w < -8] = -8
w = (w - min(w))/(max(w) - min(w))
plot(shp, col = gray(1 - w))

ind = startsWith(names(fits[[26]]$coefficients), "ne.1.")
w = fits[[26]]$coefficients[ind]
w = (w - min(w))/(max(w) - min(w))
plot(shp, col = gray(1 - w))

ind = startsWith(names(fits[[26]]$coefficients), "-log")
w = fits[[26]]$coefficients[ind]
w = 1/exp(-w)
w = w/max(w)Qcn0t2
...
tail(fits[[26]]$coefficients, 56)[1:3] + qnorm(0.025)*tail(fits[[26]]$se, 56)[1:3]
tail(fits[[26]]$coefficients, 56)[1:3] + qnorm(0.975)*tail(fits[[26]]$se, 56)[1:3]


source("PopBoundaryAPI.R")
pop = getPOP("DEU", 2012, 1000)
pop = crop(pop, as_Spatial(shp))
pop = mask(pop, as_Spatial(shp))

sum(!is.na(values(pop))) # 6 303 737

wcoef = tail(fits[[26]]$coefficients, 56)[1:3]
names(wcoef) = NULL

attract = function(d, pO, pD, coefs){
  # if((pO == 0) | (pD == 0)) return(0)
  exp(-coefs[1]*log(d) - coefs[2]*log(pO) + coefs[3]*log(pD))
}
...
attraction2 = pop
val = values(pop)
ind = which(!is.na(val) & (val > 0))
coor = coordinates(attraction)[ind, ]

origin = c(7.5, 51.5)
d = distGeo(p1 = origin, p2 = coor)
pO = extract(pop, data.frame(origin[1], origin[2]))
pD = values(pop)[ind]

values(attraction)[ind] = attract(d = d, pO = pD, pD = pD, coefs = wcoef)
values(attraction2)[ind] = attract(d = d, pO = pD, pD = pD, coefs = wcoef)*pD

# values(attraction) = log(values(attraction) + 1)
temp = values(attraction)
temp[temp > quantile(temp, 0.9999, na.rm = T)] = quantile(temp, 0.9999, na.rm = T)
temp = log(temp)
hist(temp)
values(attraction) = temp

temp = values(attraction2)
temp[temp > quantile(temp, 0.9999, na.rm = T)] = quantile(temp, 0.9999, na.rm = T)
temp = log(temp)
hist(temp)
values(attraction2) = temp
...
coefs = fits[[26]]$coefficients
ind = startsWith(names(coefs), "end.1")
w = exp(coefs[ind])
w = w / max(w)
hist(w)
plot(shp, col = gray(1 - w))
...
absBias = matrix(0, nrow = 401, ncol = 8)

for(r in 1:4){
  for(m in 1:8){
    # Relative Bias
    fittedValues = fits[[r]][[m]]$fitted
    obsValues = observed(fits[[r]][[m]]$stsObj)[fits[[r]][[m]]$control$subset, ]
    absBias[ind[[r]], m] = colMeans(abs(fittedValues - obsValues))/colMeans(fits[[r]][[m]]$stsObj@populationFrac)
    
    psi = exp(tail(fits[[r]][[m]]$coefficients, 1))
    sdev = sqrt(fittedValues + fittedValues^2/psi)
    Z = abs(obsValues - fittedValues)/sdev
    
    absBias[ind[[r]], m] = colMeans(Z)
  }
}



bestModel = apply(absBias, 1, which.min)
shp_sf$bestModel = bestModel

ggplot(data = shp_sf) +
  geom_sf(aes(fill = as.factor(bestModel)), lwd = 0.2, color = "white") +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
    axis.text = element_blank(),
...
par(mfrow = c(1, 2))
plot(shp, col = alpha(ifelse(w1 > 0, "red", "green"), w1))
plot(shp, col = alpha(ifelse(w2 > 0, "red", "green"), w2))


ggplot(data = shp_sf) +
  geom_sf(aes(fill = ifelse(w1 > 0, "Positive", "Negative"), 
              alpha = abs(w1)), lwd = 0.2, color = "white") +
  scale_fill_manual(values = c("Positive" = "red", "Negative" = "blue"), guide = guide_legend(title = "Direction")) +
  scale_alpha_continuous(range = c(0, 1), guide = guide_legend(title = "Intensity")) +
  theme_minimal() +
  theme(
    panel.grid = element_blank(),
    axis.text = element_blank(),
...
par(mfrow = c(1, 2))
plot(shp, col = alpha(cols[colsTemp], b1))
plot(shp, col = alpha(cols[colsTemp], b2))
dev.off()




# Forecasting: For each district, find best method

# For each district, find region and district ID in forecast2

plot(Ger[70, ]$geometry)
plot(shp[1])

LIS = matrix(0, nrow = 401, ncol = 8)
tempDF = forecast2[(forecast2$type == 6) & (forecast2$h == 1), ]

for(i in 1:nrow(tempDF)){
  LIS[ind[[tempDF[i, 7]]][tempDF[i, 1]], tempDF[i, 5]] = LIS[ind[[tempDF[i, 7]]][tempDF[i, 1]], tempDF[i, 5]] + tempDF[i, 6]
  if(i/1000 == floor(i/1000)) print(i)
}

LIS = LIS[, c(1, 3:8)]
apply(LIS, 1, which.min)

m = 2
plot(shp, col = gray(1 - LIS[, m]/max(LIS[, m])))



# Performance measures for different method across time

# For each region, find average performance of forecasts in a given week of the year
# Only one week ahead forecasts, only lWIS

plot(log(rowSums(cases[929:1029, indN]) + 1), type = "l")
plot(log(rowSums(cases[929:1029, indE]) + 1), type = "l")
plot(log(rowSums(cases[929:1029, indS]) + 1), type = "l")



# tEnd 928 - 1028
perfTime = matrix(0, nrow = 7*4, ncol = 101)
forecast4 = forecast2[(forecast2$h == 1) & (forecast2$type == 6), ]

for(i in 1:nrow(forecast4)){
  tt = forecast4[i, 2]
  m = forecast4[i, 5]
  m = m - (m > 1)
  r = forecast4[i, 7]
  perfTime[(r-1)*7 + m, tt - 927] = perfTime[(r-1)*7 + m, tt - 927] + forecast4[i, 6]
}

perfTime = log(perfTime + 1)

plot(perfTime[9, ], type = "l")
lines(perfTime[10, ], col = 3)
lines(perfTime[11, ], col = 4)
lines(perfTime[12, ], col = 5)
lines(perfTime[13, ], col = 6)
lines(perfTime[14, ], col = 7)

lines(log(rowSums(cases[929:1029, indW]) + 1), lwd = 2, col = 2)


for(r in 1:4){
  for(m in c(1, 3:8)){
    for(tt in 928:1028){
      ind = which((forecast2$region == r) & (forecast2$h == 1) & (forecast2$type == 6) & (forecast2$model == m) & (forecast2$tEnd == tt))
      perfTime[(r-1)*7 + m, tt - 927] = mean(forecast2$value[ind])
      print(c(r, m, tt))
    }
  }
}
